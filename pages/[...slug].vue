<template>
    <div class="grid grid-cols-1 md:grid-cols-4 bg-np-gray-50 dark:bg-np-black-500 text-np-black-500 dark:text-np-gray-50 font-sans">
        <header
            ...
            <nuxt-link to="/" class="justify-start flex">
                <logo-dark class="-ml-8 h-12" v-if="$colorMode.preference == 'dark'"></logo-dark>
                <logo-light class="-ml-8 h-12" v-if="$colorMode.preference == 'light'"></logo-light>
            </nuxt-link>
            <div class="px-8 justify-between flex flex-wrap">
                <div class="block md:hidden px-4" @click="showSidebar">menu</div>
                <ColorModePicker />
            </div>
        </header>
        <div class="grid grid-cols-1 md:grid-cols-5 col-span-4 relative">
        <aside class="col-span-1 w-full dark:border-np-black-400 bg-np-gray-100 dark:bg-np-black-500 border-np-gray-100 border-r p-8 absolute md:static -left-full"
            ...
            <ContentNavigation v-slot="{ navigation }" class="text-np-black-300 dark:text-np-gray-300">
                <div v-for="link of navigation" :key="link._path" class="py-2">
                    ...
                    <div v-for="child of link.children" :key="child._path">
                        <NuxtLink :to="child._path" class="py-8 text-lg font-serif ml-4">{{ child.title }}</NuxtLink>
                    </div>
                </div>
            </ContentNavigation>
        </aside>
